<template>
  <div v-if="showProfileMenu" class="absolute top-3 right-3 z-50">
    <div class="relative">
      <!-- Trigger Button -->
      <button
        @click="toggleMenu"
          ...
          >
            ...
            <div class="py-1.5">
              ...
              <NuxtLink
                to="/"
                @click="menuOpen = false"
                class="flex items-center gap-2.5 px-3 py-2 text-gray-700 hover:bg-slate-50 transition-colors group"
              >
                ...
                <div class="flex-1">
                  <div class="text-xs font-medium text-gray-900">All Services</div>
                  <div class="text-[10px] text-gray-500">Go to feature picker</div>
                </div>
              </NuxtLink>
...
const isSoulsRoute = computed(() => route.path.startsWith('/souls'))
const showProfileMenu = computed(() => !!user.value && !isSoulsRoute.value)
