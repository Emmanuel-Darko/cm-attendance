    ...
    <div class="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50">
      <div class="max-w-6xl mx-auto px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8">
        ...
        <div class="text-center mb-6 sm:mb-8">
          <h1
            class="text-2xl sm:text-3xl md:text-4xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent mb-2">
            Manage Sessions
          </h1>
        ...
        <!-- Create Session Card -->
        <div
          class="bg-white rounded-xl sm:rounded-2xl shadow-xl border border-indigo-100/50 p-4 sm:p-6 md:p-8 mb-6 sm:mb-8">
          <div class="flex items-center gap-3 mb-4 sm:mb-6">
            <div class="p-2 sm:p-2.5 bg-indigo-100 rounded-xl">
              <svg class="w-5 h-5 sm:w-6 sm:h-6 text-indigo-600" fill="none" stroke="currentColor" stroke-width="2"
                viewBox="0 0 24 24">
          ...
          <form @submit.prevent="createSessionHandler" class="space-y-4 sm:space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
              ...
              <div class="space-y-2">
                ...
                <input id="session-title" v-model="title" placeholder="e.g. Sunday Service - August 10, 2025"
              ...
              <div class="space-y-2">
                ...
                <input id="session-date" v-model="date" type="date"
                  class="w-full border-2 border-gray-200 rounded-lg sm:rounded-xl px-3 sm:px-4 py-2 sm:py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all text-sm sm:text-base" />
              </div>
            </div>
            ...
            <!-- Submit Button -->
            <button type="submit" :disabled="!title || !date || loading"
        ...
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-xl border border-indigo-100/50 p-4 sm:p-6 md:p-8">
          <div class="flex items-center gap-3 mb-4 sm:mb-6">
            <div class="p-2 sm:p-2.5 bg-purple-100 rounded-xl">
              <svg class="w-5 h-5 sm:w-6 sm:h-6 text-purple-600" fill="none" stroke="currentColor" stroke-width="2"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round"
          ...
          <div v-if="sessionsSorted.length === 0" class="py-12 sm:py-16 text-center">
            <div class="inline-flex p-4 bg-indigo-50 rounded-2xl mb-4">
              <svg class="w-12 h-12 sm:w-16 sm:h-16 text-indigo-300" fill="none" stroke="currentColor" stroke-width="1.5"
                viewBox="0 0 48 48">
          ...
          <div v-else class="space-y-3">
            <div v-for="session in sessionsSorted" :key="session.id"
              class="rounded-xl border-2 transition-all duration-300 overflow-hidden" :class="[
                session.is_open
              ...
              <!-- Main Card Content -->
              <div class="p-4 cursor-pointer" :class="{ 'cursor-default': session.is_open }"
                @click="!session.is_open && toggleExpand(session.id)">
                <div class="flex items-start gap-3">
                  <!-- Avatar -->
                  <div
                    class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-indigo-400 to-purple-400 flex items-center justify-center text-white font-bold text-sm sm:text-lg flex-shrink-0">
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-2 mb-2">
                      ...
                      <!-- Status Badge -->
                      <span class="flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold flex-shrink-0"
                        :class="[
                          session.is_open
                        ...
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                          <path v-if="session.is_open" stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div class="flex flex-wrap gap-2 mt-3">
                      <!-- Start Check-in for open sessions -->
                      <NuxtLink
                        v-if="session.is_open"
                        :to="`/checkin?session=${session.id}`"
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-green-600 to-indigo-500 text-white rounded-lg font-semibold text-xs sm:text-sm hover:from-green-700 hover:to-indigo-600 hover:scale-105 transition-all shadow-md"
                        style="font-weight: 600;">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round"
                            d="M5 13l4 4L19 7" />
                        </svg>
                        <span v-if="session.attendance && session.attendance.length > 0">Continue Check-in</span>
                        <span v-else>Start Check-in</span>
                      </NuxtLink>
                      <button v-if="session.is_open" @click.stop="closeSessionHandler(session.id, session.title)"
                        ...
                        <svg v-if="loadingClose === session.id" class="animate-spin h-3.5 w-3.5" fill="none"
                          viewBox="0 0 24 24">
                        ...
                        <svg v-else class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2"
                          viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                        <span class="hidden xs:inline">{{ loadingClose === session.id ? 'Closing...' : 'Close Session'
                          }}</span>
                        <span class="xs:hidden">{{ loadingClose === session.id ? 'Closing...' : 'Close' }}</span>
                      </button>

                      <button v-if="!session.is_open" @click.stop="toggleExpand(session.id)"
                        ...
                        <svg class="w-3.5 h-3.5 transition-transform"
                          :class="{ 'rotate-180': expandedSession === session.id }" fill="none" stroke="currentColor"
              ...
              <transition name="expand">
                <div v-if="!session.is_open && expandedSession === session.id"
                  class="border-t border-gray-200 bg-white/50 p-4">
                  <!-- Attendance Stats -->
                  <div class="grid grid-cols-2 gap-3 sm:gap-4 mb-4">
                    <!-- Present Kids -->
                    <div
                      class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg sm:rounded-xl p-3 sm:p-4 border-2 border-green-200">
                    ...
                    <!-- Absent Kids -->
                    <div
                      class="bg-gradient-to-br from-red-50 to-orange-50 rounded-lg sm:rounded-xl p-3 sm:p-4 border-2 border-red-200">
                      <div class="flex items-center justify-between mb-2">
                        <div class="flex items-center gap-2">
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-4">
                    ...
                    <div class="bg-white rounded-lg p-3 border border-gray-200">
                      ...
                      <p
                        class="text-xs sm:text-sm text-gray-600 transition-all duration-200"
                        :class="{'line-clamp-3': !session._showFullRecap}"
                        @click="session._showFullRecap = !session._showFullRecap"
                        style="cursor: pointer;"
                        title="Click to expand/collapse"
                      >
                        {{ session.recap || 'No sermon notes added' }}
                        <span v-if="session.recap && session.recap.length > 80" class="ml-1 text-indigo-500 underline text-xs select-none">
                          {{ session._showFullRecap ? ' (Show less)' : ' ...Show more' }}
                        </span>
                      </p>
                    </div>
                    ...
                    <div class="bg-white rounded-lg p-3 border border-gray-200 sm:col-span-2 lg:col-span-1">
                      ...
                      <div class="flex flex-wrap gap-2">
                        <template v-if="session.teachers && session.teachers.length > 0">
                          <span v-for="teacher in session.teachers" :key="teacher.id"
                            ...
                            <svg class="w-3.5 h-3.5 mr-1 text-indigo-500" fill="none" stroke="currentColor"
                              stroke-width="2" viewBox="0 0 24 24">
                              <path stroke-linecap="round" stroke-linejoin="round"
                                d="M12 6a3 3 0 110 6 3 3 0 010-6zm0 8c-3.314 0-6 1.343-6 3v1h12v-1c0-1.657-2.686-3-6-3z" />
                            </svg>
                            {{ teacher.name }}
                          </span>
                        </template>
                  ...
                  <!-- View Reports Button -->
                  <NuxtLink :to="`/attendance?session=${session.id}`"
