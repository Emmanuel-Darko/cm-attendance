              ...
              <div class="flex flex-col gap-3 mb-5">
                ...
                <div class="flex flex-col sm:flex-row gap-2">
                  <select
                    ...
                    class="w-full sm:w-64 border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all bg-gray-50 hover:bg-white text-sm sm:text-base font-medium"
                  >
                        ...
                        <div class="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p class="text-xs text-gray-500 flex items-center gap-1">
                              {{ item.id.slice(0, 12) }}...
                              <button @click.stop="copyId(item.id)" title="Copy ID" class="text-gray-400 hover:text-indigo-600 transition-colors">
...
const filtered = computed(() => {
  ...
  if (search.value) {
    ...
    list = list.filter(k =>
      k.full_name?.toLowerCase().includes(term) || k.id?.toLowerCase().includes(term)
    )
  }
  return list
})
...
const handleExportPDF = async () => {
  ...
  try {
    const selectedKidsList = Array.from(selectedKids.value)
      .map(id => adminKids.value.find(kid => kid.id === id))
